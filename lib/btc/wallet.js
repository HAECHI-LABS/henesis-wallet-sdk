...
class BtcMasterWallet extends wallet_1.Wallet {
    ...
    async build(to, amount, passphrase, feeRate) {
        common_1.checkNullAndUndefinedParameter({ to, passphrase });
        const rawTransaction = await this.createRawTransaction(to, amount, feeRate);
    ...
    async transfer(to, amount, passphrase, otpCode, feeRate) {
        return this.sendSignedTransaction(Object.assign(Object.assign({}, (await this.build(to, amount, passphrase, feeRate))), { otpCode: otpCode }));
    }
    ...
    async createRawTransaction(to, amount, feeRate) {
