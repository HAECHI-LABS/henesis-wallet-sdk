...
const bch_1 = require("../../__generate__/bch");
const walletControllerApi = (client) => {
    const apiClient = lodash_1.default.get(client, ["apiClient"]);
    return bch_1.WalletControllerApiFactory(undefined, "", apiClient);
};
exports.createDepositAddressApi = async ({ client, walletId, request, }) => {
    const response = await walletControllerApi(client).createDepositAddress2(walletId, request);
    return response.data;
};
exports.getDepositAddressApi = async ({ client, walletId, depositAddressId, }) => {
    const response = await walletControllerApi(client).getDepositAddress2(walletId, depositAddressId);
    return response.data;
};
