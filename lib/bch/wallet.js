...
exports.BchMasterWallet = exports.transformWalletData = void 0;
...
const wallet_2 = require("../apis/bch/wallet");
...
class BchMasterWallet extends wallet_1.Wallet {
    constructor(data, client, keychains, env) {
        super(client, keychains, `/wallets/${data.id}`, blockchain_1.BlockchainType.BITCOIN_CASH);
        this.data = data;
        this.env = env;
    }
    async build(to, amount, passphrase, feeRate, metadata) {
        ...
        rawTransaction.outputs.forEach((output) => {
            tx.addOutput(bitcoinjs_lib_1.address.toOutputScript(utils_1.convertToLegacyAddress(output.to), this.env === 3 ? network_1.bitcoinCashMainnet : network_1.bitcoinCashTestnet), new bn_js_1.default(output.amount.slice(2), "hex").toNumber());
        });
        const accountSigs = [];
        for (let i = 0; i < rawTransaction.inputs.length; i++) {
            const sigHash = tx.hashForWitnessV0(i, new Buffer(rawTransaction.inputs[i].redeemScript.slice(2), "hex"), rawTransaction.inputs[i].transactionOutput.amount.toNumber(), 65);
            const hexHash = this.keychains.sign(this.data.accountKey, passphrase, sigHash.toString("hex"));
            const accountSig = utils_1.encodeScriptSignature(Buffer.from(hexHash, "hex"), 65).toString("hex");
    ...
    async changeName(name) {
        ...
        const bchWalletData = await this.client.patch(`${this.baseUrl}/name`, request);
...
exports.BchMasterWallet = BchMasterWallet;
