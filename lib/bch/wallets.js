...
class BchWallets extends wallets_1.Wallets {
    ...
    async createMasterWallet(name, passphrase) {
        ...
        return new wallet_1.BchMasterWallet(wallet_1.transformWalletData(data), this.client, this.keychains, this.env);
    }
    async getWallet(id) {
    ...
    verifyAddress(address) {
        ...
        try {
            ...
            if (utils_1.isNewAddress(address)) {
                targetAddress = utils_1.convertToLegacyAddress(address);
            }
            bitcoinjs_lib_1.address.toOutputScript(targetAddress, this.env === 3 ? network_1.bitcoinCashMainnet : network_1.bitcoinCashTestnet);
            return true;
        }
        catch (e) {
            return false;
    ...
    async getMasterWallets(options) {
        ...
        return walletDatas.map((wallet) => new wallet_1.BchMasterWallet(wallet_1.transformWalletData(wallet), this.client, this.keychains, this.env));
    }
    async createRecoveryKit(name, passphrase) {
        ...
        return new recoveryKit_1.BchRecoveryKit(name, blockchain_1.BlockchainType.BITCOIN_CASH, masterWalletResponse.henesisKey, accountKey, backupKey, js_base64_1.Base64.encode(encryptedPassphrase), aes_js_1.default.utils.hex.fromBytes(encryptionKeyBuffer), this.env, masterWalletResponse.id);
    }
    async createMasterWalletWithKit(recoveryKit) {
        ...
        return new wallet_1.BchMasterWallet(wallet_1.transformWalletData(wallet), this.client, this.keychains, this.env);
    }
    async createInactiveMasterWallet(name) {
        ...
        return {
            id,
            name: walletName,
            blockchain: blockchain_1.BlockchainType.BITCOIN_CASH,
            henesisKey: {
