...
class Wallet {
    ...
    async changePassphrase(passphrase, newPassphrase, otpCode) {
        ...
        return await this.changePassphraseWithKeyFile(passphrase, newPassphrase, undefined, otpCode);
    }
    async changePassphraseWithKeyFile(passphrase, newPassphrase, initialKey, otpCode) {
        const newKey = this.keychains.changePassword(initialKey ? initialKey : this.getAccountKey(), passphrase, newPassphrase);
