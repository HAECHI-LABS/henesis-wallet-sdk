...
sjcl.cipher.aes.prototype = {
    ...
    O() {
        ...
        for (f = g = 0; !c[f]; f ^= l || 1, g = k[g] || 1)
            for (m = g ^ g << 1 ^ g << 2 ^ g << 3 ^ g << 4, m = m >> 8 ^ m & 255 ^ 99, c[f] = m, d[m] = f, n = h[e = h[l = h[f]]], p = 0x1010101 * n ^ 0x10001 * e ^ 0x101 * l ^ 0x1010100 * f, n = 0x101 * h[m] ^ 0x1010100 * m, e = 0; e < 4; e++)
...
sjcl.bitArray = {
    ...
    concat(a, b) { if (a.length === 0 || b.length === 0)
        return a.concat(b); const c = a[a.length - 1]; const d = sjcl.bitArray.getPartial(c); return d === 32 ? a.concat(b) : sjcl.bitArray.$(b, d, c | 0, a.slice(0, a.length - 1)); },
    bitLength(a) {
        const b = a.length;
        return b
            === 0 ? 0 : 32 * (b - 1) + sjcl.bitArray.getPartial(a[b - 1]);
    },
    clamp(a, b) { if (32 * a.length < b)
        return a; a = a.slice(0, Math.ceil(b / 32)); const c = a.length; b &= 31; c > 0 && b && (a[c - 1] = sjcl.bitArray.partial(b, a[c - 1] & 2147483648 >> b - 1, 1)); return a; },
    ...
    $(a, b, c, d) { let e; e = 0; for (void 0 === d && (d = []); b >= 32; b -= 32)
        d.push(c), c = 0; if (b === 0)
        return d.concat(a); for (e = 0; e < a.length; e++)
        d.push(c | a[e] >>> b), c = a[e] << 32 - b; e = a.length ? a[a.length - 1] : 0; a = sjcl.bitArray.getPartial(e); d.push(sjcl.bitArray.partial(b + a & 31, b + a > 32 ? c : d.pop(), 1)); return d; },
    i(a, b) { return [a[0] ^ b[0], a[1] ^ b[1], a[2] ^ b[2], a[3] ^ b[3]]; },
    byteswapM(a) { let b; let c; for (b = 0; b < a.length; ++b)
        c = a[b], a[b] = c >>> 24 | c >>> 8 & 0xff00 | (c & 0xff00) << 8 | c << 24; return a; },
};
sjcl.codec.utf8String = { fromBits(a) { let b = ''; const c = sjcl.bitArray.bitLength(a); let d; let e; for (d = 0; d < c / 8; d++)
        (d & 3) === 0 && (e = a[d / 4]), b += String.fromCharCode(e >>> 8 >>> 8 >>> 8), e <<= 8; return decodeURIComponent(escape(b)); }, toBits(a) { a = unescape(encodeURIComponent(a)); const b = []; let c; let d = 0; for (c = 0; c < a.length; c++)
        d = d << 8 | a.charCodeAt(c), (c & 3) === 3 && (b.push(d), d = 0); c & 3 && b.push(sjcl.bitArray.partial(8 * (c & 3), d)); return b; } };
sjcl.codec.hex = { fromBits(a) { let b = ''; let c; for (c = 0; c < a.length; c++)
        b += ((a[c] | 0) + 0xf00000000000).toString(16).substr(4); return b.substr(0, sjcl.bitArray.bitLength(a) / 4); }, toBits(a) { let b; const c = []; let d; a = a.replace(/\s|0x/g, ''); d = a.length; a += '00000000'; for (b = 0; b < a.length; b += 8)
...
sjcl.codec.base32 = {
    ...
    BASE: 5,
    REMAINING: 27,
    fromBits(a, b, c) { const d = sjcl.codec.base32.BASE; const e = sjcl.codec.base32.REMAINING; let f = ''; let g = 0; let h = sjcl.codec.base32.B; let k = 0; const l = sjcl.bitArray.bitLength(a); c && (h = sjcl.codec.base32.X); for (c = 0; f.length * d < l;)
        f += h.charAt((k ^ a[c] >>> g) >>> e), g < d ? (k = a[c] << d - g, g += e, c++) : (k <<= d, g -= d); for (; f.length & 7 && !b;)
        f += '='; return f; },
    toBits(a, b) {
        a = a.replace(/\s|=/g, '').toUpperCase();
        const c = sjcl.codec.base32.BITS;
        const d = sjcl.codec.base32.BASE;
        const e = sjcl.codec.base32.REMAINING;
        const f = [];
        let g;
        let h = 0;
        let k = sjcl.codec.base32.B;
        let l = 0;
        let n;
        let m = 'base32';
...
sjcl.codec.base64 = {
    ...
    fromBits(a, b, c) { let d = ''; let e = 0; let f = sjcl.codec.base64.B; let g = 0; const h = sjcl.bitArray.bitLength(a); c && (f = `${f.substr(0, 62)}-_`); for (c = 0; 6 * d.length < h;)
        d += f.charAt((g ^ a[c] >>> e) >>> 26), e < 6 ? (g = a[c] << 6 - e, e += 26, c++) : (g <<= 6, e -= 6); for (; d.length & 3 && !b;)
        d += '='; return d; },
    toBits(a, b) {
        a = a.replace(/\s|=/g, '');
        const c = [];
...
sjcl.codec.base64url = { fromBits(a) { return sjcl.codec.base64.fromBits(a, 1, 1); }, toBits(a) { return sjcl.codec.base64.toBits(a, 1); } };
...
sjcl.hash.sha256.prototype = {
    ...
    update(a) {
        ...
        let b;
        const c = this.A = sjcl.bitArray.concat(this.A, a);
        b = this.l;
        a = this.l = b + sjcl.bitArray.bitLength(a);
        if (a > 0x1fffffffffffff)
    ...
    finalize() { let a; var b = this.A; const c = this.F; var b = sjcl.bitArray.concat(b, [sjcl.bitArray.partial(1, 1)]); for (a = b.length + 2; a & 15; a++)
        b.push(0); b.push(Math.floor(this.l / 0x100000000)); for (b.push(this.l | 0); b.length;)
        u(this, b.splice(0, 16)); this.reset(); return c; },
    Y: [],
    b: [],
    O() {
        function a(a) { return 0x100000000 * (a - Math.floor(a)) | 0; }
        for (var b = 0, c = 2, d, e; b < 64; c++) {
            e = !0;
            for (d = 2; d * d <= c; d++) {
                if (c % d === 0) {
                    e = !1;
...
function u(a, b) {
    ...
    let q = f[7];
    for (c = 0; c < 64; c++)
        c < 16 ? d = b[c] : (d = b[c + 1 & 15], e = b[c + 14 & 15], d = b[c & 15] = (d >>> 7 ^ d >>> 18 ^ d >>> 3 ^ d << 25 ^ d << 14) + (e >>> 17 ^ e >>> 19 ^ e >>> 10 ^ e << 15 ^ e << 13) + b[c & 15] + b[c + 9 & 15] | 0), d = d + q + (m >>> 6 ^ m >>> 11 ^ m >>> 25 ^ m << 26 ^ m << 21 ^ m << 7) + (r ^ m & (p ^ r)) + g[c], q = r, r = p, p = m, m = n + d | 0, n = l, l = k, k = h, h = d + (k & l ^ n & (k ^ l)) + (k >>> 2 ^ k >>> 13 ^ k >>> 22 ^ k << 30 ^ k << 19 ^ k << 10) | 0;
    f[0] = f[0] + h | 0;
    f[1] = f[1] + k | 0;
    f[2] = f[2] + l | 0;
    f[3] = f[3] + n | 0;
...
sjcl.mode.ccm = {
    ...
    fa(a) { const b = sjcl.mode.ccm.G.slice(); let c; for (c = 0; c < b.length; c += 1)
        b[c](a); },
    encrypt(a, b, c, d, e) {
        let f;
        let g = b.slice(0);
        const h = sjcl.bitArray;
        const k = h.bitLength(c) / 8;
        const l = h.bitLength(g) / 8;
        e = e || 64;
        d = d || [];
        if (k < 7)
    ...
    decrypt(a, b, c, d, e) {
        e = e || 64;
        d = d || [];
        const f = sjcl.bitArray;
        const g = f.bitLength(c) / 8;
        var h = f.bitLength(b);
        let k = f.clamp(b, h - e);
        const l = f.bitSlice(b, h - e);
        var h = (h - e) / 8;
        if (g < 7)
            throw new sjcl.exception.invalid('ccm: iv must be at least 7 bytes');
        for (b = 2; b < 4 && h >>> 8 * b; b++)
    ...
    na(a, b, c, d, e, f) { let g = []; const h = sjcl.bitArray; const k = h.i; d = [h.partial(8, (b.length ? 64 : 0) | d - 2 << 2 | f - 1)]; d = h.concat(d, c); d[3] |= e; d = a.encrypt(d); if (b.length)
        for (c = h.bitLength(b) / 8, c <= 65279 ? g = [h.partial(16, c)] : c <= 0xffffffff && (g = h.concat([h.partial(16, 65534)], [c])), g = h.concat(g, b), b = 0; b < g.length; b += 4)
            d = a.encrypt(k(d, g.slice(b, b + 4).concat([0, 0, 0]))); return d; },
    V(a, b, c, d, e, f) {
        const g = sjcl.bitArray;
        const h = g.i;
        e /= 8;
        if (e % 2 || e < 4 || e > 16)
...
sjcl.mode.ocb2 = {
    ...
    encrypt(a, b, c, d, e, f) {
        ...
        let g;
        const h = sjcl.mode.ocb2.S;
        const k = sjcl.bitArray;
        const l = k.i;
        let n = [0, 0, 0, 0];
        c = h(a.encrypt(c));
        let m;
    ...
    decrypt(a, b, c, d, e, f) {
        ...
        let m;
        let p;
        const r = sjcl.bitArray.bitLength(b) - e;
        let q = [];
        d = d || [];
    ...
    pmac(a, b) {
        let c;
        const d = sjcl.mode.ocb2.S;
        const e = sjcl.bitArray;
        const f = e.i;
        let g = [0, 0, 0, 0];
...
sjcl.mode.gcm = {
    ...
    decrypt(a, b, c, d, e) { let f = b.slice(0); const g = sjcl.bitArray; const h = g.bitLength(f); e = e || 128; d = d || []; e <= h ? (b = g.bitSlice(f, h - e), f = g.bitSlice(f, 0, h - e)) : (b = f, f = []); a = sjcl.mode.gcm.C(!1, a, f, d, c, e); if (!g.equal(a.tag, b))
        throw new sjcl.exception.corrupt("gcm: tag doesn't match"); return a.data; },
    ka(a, b) {
        let c;
        let d;
        let e;
    ...
    j(a, b, c) { let d; const e = c.length; b = b.slice(0); for (d = 0; d < e; d += 4)
        b[0] ^= 0xffffffff & c[d], b[1] ^= 0xffffffff & c[d + 1], b[2] ^= 0xffffffff & c[d + 2], b[3] ^= 0xffffffff & c[d + 3], b = sjcl.mode.gcm.ka(b, a); return b; },
    C(a, b, c, d, e, f) {
        let g;
        let h;
        let k;
        ...
        let p;
        let r;
        const q = sjcl.bitArray;
        m = c.length;
        p = q.bitLength(c);
        r = q.bitLength(d);
        h = q.bitLength(e);
        g = b.encrypt([0, 0, 0, 0]);
        h === 96 ? (e = e.slice(0), e = q.concat(e, [1])) : (e = sjcl.mode.gcm.j(g, [0, 0, 0, 0], e), e = sjcl.mode.gcm.j(g, e, [0, 0, Math.floor(h / 0x100000000), h & 0xffffffff]));
...
sjcl.misc.hmac = function (a, b) { this.W = b = b || sjcl.hash.sha256; const c = [[], []]; let d; const e = b.prototype.blockSize / 32; this.w = [new b(), new b()]; a.length > e && (a = b.hash(a)); for (d = 0; d < e; d++)
...
sjcl.misc.hmac.prototype.digest = function () { var a = this.R.finalize(); var a = (new this.W(this.w[1])).update(a).finalize(); this.reset(); return a; };
sjcl.misc.pbkdf2 = function (a, b, c, d, e) { c = c || 1E4; if (d < 0 || c < 0)
    throw new sjcl.exception.invalid('invalid params to pbkdf2'); typeof a === 'string' && (a = sjcl.codec.utf8String.toBits(a)); typeof b === 'string' && (b = sjcl.codec.utf8String.toBits(b)); e = e || sjcl.misc.hmac; a = new e(a); let f; let g; let h; let k; let l = []; const n = sjcl.bitArray; for (k = 1; 32 * l.length < (d || 1); k++) {
...
sjcl.prng.prototype = {
    randomWords(a, b) {
        ...
        if (d & this.J) {
            ...
            for (d = 0; d < 4 && (this.h[d] = this.h[d] + 1 | 0, !this.h[d]); d++)
                ;
        }
        for (d = 0; d < a; d += 4)
    ...
    setDefaultParanoia(a, b) { if (a === 0 && b !== 'Setting paranoia=0 will ruin your security; use it only for testing')
        throw new sjcl.exception.invalid('Setting paranoia=0 will ruin your security; use it only for testing'); this.M = a; },
    addEntropy(a, b, c) {
        c = c || 'user';
        let d;
        let e;
    ...
    removeEventListener(a, b) { let c; let d; const e = this.K[a]; const f = []; for (d in e)
        e.hasOwnProperty(d) && e[d] === b && f.push(d); for (c = 0; c < f.length; c++)
        d = f[c], delete e[d]; },
    la() { C(this, 1); },
    oa(a) { let b; let c; try {
        b = a.x || a.clientX || a.offsetX || 0, c = a.y || a.clientY || a.offsetY || 0;
    }
    catch (d) {
        c = b = 0;
    } b != 0 && c != 0 && this.addEntropy([b, c], 2, 'mouse'); C(this, 0); },
    qa(a) {
    ...
    ma() { C(this, 2); },
    ea(a) { a = a.accelerationIncludingGravity.x || a.accelerationIncludingGravity.y || a.accelerationIncludingGravity.z; if (window.orientation) {
        const b = window.orientation;
        typeof b === 'number' && this.addEntropy(b, 1, 'accelerometer');
    } a && this.addEntropy(a, 2, 'accelerometer'); C(this, 0); },
};
function A(a, b) { let c; const d = sjcl.random.K[a]; const e = []; for (c in d)
    d.hasOwnProperty(c) && e.push(d[c]); for (c = 0; c < e.length; c++)
    e[c](b); }
function C(a, b) { typeof window !== 'undefined' && window.performance && typeof window.performance.now === 'function' ? a.addEntropy(window.performance.now(), b, 'loadtime') : a.addEntropy((new Date()).valueOf(), b, 'loadtime'); }
function y(a) { a.b = z(a).concat(z(a)); a.L = new sjcl.cipher.aes(a.b); }
function z(a) { for (let b = 0; b < 4 && (a.h[b] = a.h[b] + 1 | 0, !a.h[b]); b++)
...
sjcl.json = {
    ...
    ja(a, b, c, d) {
        ...
        d = d || {};
        const e = sjcl.json;
        const f = e.g({ iv: sjcl.random.randomWords(4, 0) }, e.defaults);
        let g;
        e.g(f, c);
        c = f.adata;
        typeof f.salt === 'string' && (f.salt = sjcl.codec.base64.toBits(f.salt));
        typeof f.iv === 'string' && (f.iv = sjcl.codec.base64.toBits(f.iv));
        if (!sjcl.mode[f.mode] || !sjcl.cipher[f.cipher] || typeof a === 'string' && f.iter <= 100 || f.ts !== 64 && f.ts !== 96 && f.ts !== 128 || f.ks !== 128 && f.ks !== 192 && f.ks !== 0x100 || f.iv.length < 2
            || f.iv.length > 4)
            throw new sjcl.exception.invalid('json encrypt: invalid parameters');
        typeof a === 'string' ? (g = sjcl.misc.cachedPbkdf2(a, f), a = g.key.slice(0, f.ks / 32), f.salt = g.salt) : sjcl.ecc && a instanceof sjcl.ecc.elGamal.publicKey && (g = a.kem(), f.kemtag = g.tag, a = g.key.slice(0, f.ks / 32));
        typeof b === 'string' && (b = sjcl.codec.utf8String.toBits(b));
        typeof c === 'string' && (f.adata = c = sjcl.codec.utf8String.toBits(c));
        g = new sjcl.cipher[f.cipher](a);
        e.g(d, f);
        d.key = a;
        f.ct = f.mode === 'ccm' && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm
    ...
    ia(a, b, c, d) {
        ...
        if (!sjcl.mode[b.mode] || !sjcl.cipher[b.cipher] || typeof a
            === 'string' && b.iter <= 100 || b.ts !== 64 && b.ts !== 96 && b.ts !== 128 || b.ks !== 128 && b.ks !== 192 && b.ks !== 0x100 || !b.iv || b.iv.length < 2 || b.iv.length > 4)
            throw new sjcl.exception.invalid('json decrypt: invalid parameters');
        typeof a === 'string' ? (g = sjcl.misc.cachedPbkdf2(a, b), a = g.key.slice(0, b.ks / 32), b.salt = g.salt) : sjcl.ecc && a instanceof sjcl.ecc.elGamal.secretKey && (a = a.unkem(sjcl.codec.base64.toBits(b.kemtag)).slice(0, b.ks / 32));
        typeof f === 'string' && (f = sjcl.codec.utf8String.toBits(f));
        g = new sjcl.cipher[b.cipher](a);
        f = b.mode
            === 'ccm' && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm && b.ct instanceof ArrayBuffer ? sjcl.arrayBuffer.ccm.decrypt(g, b.ct, b.iv, b.tag, f, b.ts) : sjcl.mode[b.mode].decrypt(g, b.ct, b.iv, f, b.ts);
        e.g(d, b);
        d.key = a;
        return c.raw === 1 ? f : sjcl.codec.utf8String.fromBits(f);
    },
    ...
    g(a, b, c) { void 0 === a && (a = {}); if (void 0 === b)
        return a; for (const d in b)
        if (b.hasOwnProperty(d)) {
            if (c && void 0 !== a[d] && a[d] !== b[d])
                throw new sjcl.exception.invalid('required parameter overridden');
            a[d] = b[d];
        } return a; },
    sa(a, b) { const c = {}; let d; for (d in a)
        a.hasOwnProperty(d) && a[d] !== b[d] && (c[d] = a[d]); return c; },
    ra(a, b) { const c = {}; let d; for (d = 0; d < b.length; d++)
        void 0 !== a[b[d]] && (c[b[d]] = a[b[d]]); return c; },
};
sjcl.encrypt = sjcl.json.encrypt;
sjcl.decrypt = sjcl.json.decrypt;
sjcl.misc.pa = {};
sjcl.misc.cachedPbkdf2 = function (a, b) { let c = sjcl.misc.pa; let d; b = b || {}; d = b.iter || 1E3; c = c[a] = c[a] || {}; d = c[d] = c[d] || { firstSalt: b.salt && b.salt.length ? b.salt.slice(0) : sjcl.random.randomWords(2, 0) }; c = void 0 === b.salt ? d.firstSalt : b.salt; d[c] = d[c] || sjcl.misc.pbkdf2(a, c, b.iter); return { key: d[c].slice(0), salt: c.slice(0) }; };
typeof module !== 'undefined' && module.exports && (module.exports = sjcl);
typeof define === 'function' && define([], () => sjcl);
