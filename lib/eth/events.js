...
class EthEvents {
    ...
    async getNftTransferEvents(options) {
        const queryString = url_1.makeQueryString(options);
        const data = await this.client.get(`/nft-transfer-events${queryString ? `?${queryString}` : ""}`);
        return {
            pagination: data.pagination,
            results: data.results.map((e) => {
                return Object.assign(Object.assign({}, e), { confirmation: common_1.BNConverter.hexStringToBN(String(e.confirmation)) });
            }),
        };
    }
    ...
    async getInternalNftTransferEvents(options) {
        const queryString = url_1.makeQueryString(options);
        const data = await this.client.get(`/internal/nft-transfer-events${queryString ? `?${queryString}` : ""}`);
        return {
            pagination: data.pagination,
            results: data.results.map((e) => {
                return Object.assign(Object.assign({}, e), { transaction: this.convertSimplifiedTransaction(e.transaction), confirmation: common_1.BNConverter.hexStringToBN(String(e.confirmation)) });
            }),
        };
    }
