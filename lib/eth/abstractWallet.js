...
class EthLikeWallet extends wallet_1.Wallet {
    ...
    async contractCall(contractAddress, value, data, passphrase, otpCode, gasPrice, gasLimit) {
        return this.sendTransaction(await this.buildContractCallPayload(contractAddress, value, data, passphrase), this.getId(), otpCode, gasPrice, gasLimit || this.DEFAULT_CONTRACT_CALL_GAS_LIMIT);
    }
    ...
    async transfer(coin, to, amount, passphrase, otpCode, gasPrice, gasLimit) {
        const c = typeof coin === "string" ? await this.coins.getCoin(coin) : coin;
        return this.sendTransaction(await this.buildTransferPayload(c, to, amount, passphrase), this.getId(), otpCode, gasPrice, gasLimit || this.getGasLimitByTicker(c));
    }
    ...
    async sendTransaction(signedMultiSigPayload, walletId, otpCode, gasPrice, gasLimit) {
        const request = {
            walletId,
