...
const __1 = require("..");
class BtcWallets extends wallets_1.Wallets {
    ...
    async createMasterWallet(name, passphrase) {
        ...
        return new wallet_1.BtcMasterWallet(wallet_1.transformWalletData(data), this.client, this.keychains, this.env);
    }
    async getWallet(id) {
    ...
    verifyAddress(address) {
        ...
        try {
            bitcoinjs_lib_1.address.toOutputScript(address, this.env === 3 ? bitcoinjs_lib_1.networks.bitcoin : bitcoinjs_lib_1.networks.testnet);
            return true;
        }
        catch (e) {
            return false;
    ...
    async getMasterWallets(options) {
        ...
        return walletDatas.map((wallet) => new wallet_1.BtcMasterWallet(wallet_1.transformWalletData(wallet), this.client, this.keychains, this.env));
    }
    async createRecoveryKit(name, passphrase) {
        ...
        return new recoveryKit_1.BtcRecoveryKit(name, blockchain_1.BlockchainType.BITCOIN, masterWalletResponse.henesisKey, accountKey, backupKey, js_base64_1.Base64.encode(encryptedPassphrase), aes_js_1.default.utils.hex.fromBytes(encryptionKeyBuffer), this.env, masterWalletResponse.id);
    }
    async createMasterWalletWithKit(recoveryKit) {
        ...
        return new wallet_1.BtcMasterWallet(wallet_1.transformWalletData(wallet), this.client, this.keychains, this.env);
    }
