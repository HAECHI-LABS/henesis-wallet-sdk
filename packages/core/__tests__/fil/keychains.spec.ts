...
describe('FilKeychains',() => {
  ...
  const expectIsSameWhenDecrypt = (key: Key, password: string) => {
    const decryptedSeed = keychains.decrypt(key, password);
    const decryptedPriv = keychains.decrypt(key, password, true);

    expect(decryptedSeed).toEqual(seed)
    expect(decryptedPriv).toEqual(priv);
  }
  ...
  describe('#changePassword()', () => {
    ...
    it('should change key password twice, so recover to original password', () => {
      ...
      expectIsSameWhenDecrypt(newKey, 'newpassword');
      ...
      expectIsSameWhenDecrypt(recoveredKey, 'password');
    })
  });
})
