...
describe('Wallets', () => {
  describe('#createMasterWallet()', () => {
    ...
    it('should be able to create master wallet', async () => {
      ...
      nock(baseUrl).post('/api/v1/wallets').reply(200, response);
      const keysResponse = {
        access_token: 'string',
        henesis_eth_key: {
          address: '0x716ba9752bbf769428dcd59352e4c16f64a2a856',
      ...
      nock(baseUrl).get('/api/v1/organizations/me').reply(200, keysResponse);
      const masterWallet: MasterWallet = await sdk.wallets.createMasterWallet(
        'klaytn_test_masterWallet',
  ...
  describe('#getMasterWallets()', () => {
    ...
    it('should return array of master wallets', async () => {
      ...
      nock(baseUrl).get('/api/v1/wallets').reply(200, response);
