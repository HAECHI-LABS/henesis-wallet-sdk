...
export class TransactionsService {
  ...
  public async getTransactions(
    ...
    const transactions = await sdk.klay.transactions.getTransactions(options);

    transactions.pagination.nextUrl = changeUrlHost(
      transactions.pagination.nextUrl,
      request
    );
    transactions.pagination.previousUrl = changeUrlHost(
      transactions.pagination.previousUrl,
      request
    );
    return {
  ...
  public async getTransaction(
    ...
    return TransactionDTO.fromTransaction(
      await sdk.klay.transactions.getTransaction(transactionId)
    );
  }
}
